...
class Analysis(Load):
    ...
    def E_Spectrum(self,viewer):
        ...
        # plt.savefig(self.path+'\Energy_spectrum.jpg')
        '''
    ...
    def Powerspectrum(self,lgc,interval,tr):
        ...
        if tr:
            ax1.plot(avg_ps1.freq,avg_ps1.freq*avg_ps1.power,c='k',label='developed')
        else:
        ...
        plt.xscale('log')
        plt.title('avg_PSD')
        #plt.savefig('avg_PSD.png')
    

    def LagFreq(self,lgc1,lgc2,interval):

        lc9 = Lightcurve(self.times, lgc1[:])
        lc10 = Lightcurve(self.times,lgc2[:])
        
        avg_cs = AveragedCrossspectrum(lc10, lc9,interval)
        '''
...
exp1.E_Spectrum(2)
lgc1 = exp1.LightCurve(1e-3,0.1,2)
lgc2 = exp1.LightCurve(0.1,1.,2)
exp1.LagFreq(lgc1[0,:],lgc2[0,:],2)
